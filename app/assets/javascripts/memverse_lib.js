...
function validVerseRef(verseref) {
    return /([0-3]?\s+)?[a-záéíóúüñäõ\uAC00-\uD7A3]+\s+[0-9]+(:(\s)?|(\s?vs\s?))[0-9]+$/i.test(verseref);
}
...
function validPassageRef(passage) {
    return /([0-3]?\s+)?[a-záéíóúüñäõ\uAC00-\uD7A3]+\s+[0-9]+(((:(\s)?|(\s?vs\s?))[0-9]+(-)[0-9]+)|:)?$/i.test(passage);
}
...
function validChapterRef(passage) {
    return /([0-3]?\s+)?[a-záéíóúüñäõ\uAC00-\uD7A3]+\s+[0-9]+(:)?$/i.test(passage);
}
...
function validSubChapterPassage(passage) {
    return /([0-3]?\s+)?[a-záéíóúüñäõ\uAC00-\uD7A3]+\s+[0-9]+(:|(\s?vs\s?))[0-9]+(-)[0-9]+/i.test(passage);
}
