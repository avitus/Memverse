...
class QuizQuestion < ActiveRecord::Base
  ...
  # Relationships
  belongs_to :quiz
  has_many :quiz_answers, :through => :livequizzes
    
  # Validations
  # validates_presence_of :user_id

  after_create 'self.quiz.compute_length'
  after_update 'self.quiz.compute_length'
  after_destroy 'self.quiz.compute_length'

  
	def passage_translations
		
		passages = Hash.new
		quiz_translations = MAJORS.keys.collect { |k| k.to_s }
