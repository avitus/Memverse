...
describe MemversesController do
  ...
  describe "GET 'ajax_add'" do
    ...
    it "should not allow the same verse in two different translations" do
      ...
      get :ajax_add, :id => @verse_esv      # This test is currently failing since it can't save the Memverse object it makes with this request
                                            # Why does this cause the test to fail? It doesn't throw an exception so the test should pass.
      @user.memverses.count.should == 1            
    end
  ...
  describe "GET 'add_chapter'" do
    ...
    before (:each) do
      @chapter = Array.new         
      for i in 1..6
        verse       = Factory(:verse, :book_index => 19, :book => "Psalms", :chapter => '1', :versenum => i, :translation => "NIV")
        @chapter[i] = Factory(:memverse, :user => @user, :verse => verse)
      end        
    end
     
    it "should add an entire chapter to users memory verses" do
      get :add_chapter, :bk => "Psalms", :ch => 1, :tl => "NIV"
      @user.memverses.count.should == 6
    end
